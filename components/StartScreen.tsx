      ...
      {!userImageUrl ? (
        ...
        >
          ...
          <div className="w-full lg:w-1/2 flex flex-col items-center justify-center">
            <Compare
              firstImage="https://res.cloudinary.com/dbylka4xx/image/upload/v1758883142/Screenshot_2025-09-26_at_3.02.42_PM_enibzi.png"
              secondImage="https://res.cloudinary.com/dbylka4xx/image/upload/v1758883251/Model_2_mkz9aw.png"
              slideMode="drag"
        ...
        >
          <div className="md:w-1/2 flex-shrink-0 flex flex-col items-center md:items-start">
            <div className="text-center md:text-left">
              ...
              <p className="mt-2 text-md text-brand-dark/80">
                Drag the slider to see your transformation.
              </p>
            </div>
            
            {isGenerating && (
              <div className="flex items-center gap-3 text-lg text-brand-dark font-serif mt-6">
                <Spinner />
            ...
            {error && 
              <div className="text-center md:text-left text-brand-red max-w-md mt-6">
                <p className="font-semibold">Generation Failed</p>
                <p className="text-sm mb-4">{error}</p>
                <button onClick={reset} className="text-sm font-semibold text-brand-dark hover:underline">Try Again</button>
              </div>
            }
            ...
            <AnimatePresence>
              {generatedModelUrl && !isGenerating && !error && (
                ...
                >
                  <button 
                    onClick={reset}
                    className="w-full sm:w-auto px-6 py-3 text-base font-semibold text-brand-dark bg-gray-200 rounded-md cursor-pointer hover:bg-gray-300 transition-colors"
                  >
                    Use Different Photo
                  </button>
                  <button 
                    onClick={() => onModelFinalized(generatedModelUrl)}
                    className="w-full sm:w-auto relative inline-flex items-center justify-center px-8 py-3 text-base font-semibold text-white bg-brand-blue rounded-md cursor-pointer group hover:bg-brand-blue/90 transition-colors"
                  >
