...
const Canvas: React.FC<CanvasProps> = ({ displayImageUrl, onStartOver, isLoading, loadingMessage, onSelectPose, poseInstructions, currentPoseIndex, availablePoseKeys, isInteractionDisabled = false }) => {
  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-4 relative animate-zoom-in group">
      ...
      <button 
          onClick={onStartOver}
          className="absolute top-4 left-4 z-30 flex items-center justify-center text-center bg-white/60 border border-brand-dark/20 text-brand-dark font-semibold py-2 px-4 rounded-full transition-all duration-200 ease-in-out hover:bg-white hover:border-brand-dark/40 active:scale-95 text-sm backdrop-blur-sm"
      >
        ...
        <AnimatePresence>
          {isLoading && (
              ...
              >
                  <Spinner />
                  {loadingMessage && (
                      <p className="text-lg font-serif text-brand-dark/90 mt-4 text-center px-4">{loadingMessage}</p>
                  )}
              </motion.div>
          )}
        </AnimatePresence>
      ...
      {displayImageUrl && !isLoading && (
        ...
        >
          ...
          <AnimatePresence>
              {isPoseMenuOpen && (
                  <motion.div
                      ...
                      className="absolute bottom-full mb-3 w-64 bg-white/80 backdrop-blur-lg rounded-xl p-2 border border-brand-dark/10"
                  >
          ...
          <div className="flex items-center justify-center gap-2 bg-white/60 backdrop-blur-md rounded-full p-2 border border-brand-dark/10">
            <button 
