...
const OutfitStack: React.FC<OutfitStackProps> = ({ outfitHistory, onRemoveLastGarment }) => {
  return (
    <div className="flex flex-col">
      <h2 className="text-xl font-serif tracking-wider text-brand-dark border-b border-brand-dark/20 pb-2 mb-3">Outfit Stack</h2>
      <div className="space-y-2">
        {outfitHistory.map((layer, index) => (
          <div
            key={layer.garment?.id || 'base'}
            className="flex items-center justify-between bg-white/50 p-2 rounded-lg animate-fade-in border border-brand-dark/10"
          >
            <div className="flex items-center overflow-hidden">
                <span className="flex-shrink-0 flex items-center justify-center w-6 h-6 mr-3 text-xs font-bold text-brand-blue bg-brand-blue/10 rounded-full">
                  {index + 1}
                </span>
                {layer.garment && (
            ...
            {index > 0 && index === outfitHistory.length - 1 && (
               <button
                ...
                className="flex-shrink-0 text-brand-dark/60 hover:text-brand-red transition-colors p-2 rounded-md hover:bg-brand-red/10"
